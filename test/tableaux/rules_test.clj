...
(deftest tableau-rules
  (testing "Basic tableau rules"
    ...
    (is (> (count (get (log-labels-by-prefix '[0 (:box p)])
                       (rule-box1)))
           0))
    (is (= (-> (rw/logging-system [log-edges-by-idx-src])
               (rw/post #{'(a 1 2) '(a 1 3)})
               (rule-box1 '(1 (:box a p)))
               u/foldset)
           #{[2 'p] [3 'p]}))
    (is (> (count (get (log-by-arity '[:a 0 1])
                       (rule-box2)))
           0))
    (is (= (-> (rw/logging-system [log-labels-by-node-prefix-infix])
               (rw/post #{'(1 (:box a p)) [2]})
               (rule-box2 '(a 1 2))
               u/foldset)
           #{[2 'p]}))
    ...
    (is (= (with-redefs [gensym (fn [x] 2)]
           ...
           #{[2] ['a 1 2] [2 [:not 'p]]}))
    (is (= (with-redefs [gensym (fn [x] 2)]
                        (-> (rw/logging-system [log-edges-by-idx-src
                                                  log-by-arity])
                            (rw/post ['[a 1 2]])
                            (rule-not-box [1 '(:not (:box a p))])
                            u/foldset))
  ...
  (testing "Precondition rules"
    (is (= (-> (rw/logging-system [log-by-arity
